...
const getItems = async () => {
 if(localStorage.getItem('userInfo')){
  let response = await Axios.get(`/api/cart/cart-items/${JSON.parse(localStorage.getItem('userInfo'))._id}`)
   return(response.data)
 }else{
   if(localStorage.getItem('cartId')){
    let response = await Axios.get(`/api/cart/cart-items/${localStorage.getItem('cartId')}`)
    return(response.data)
   }else{
     return []
   }
